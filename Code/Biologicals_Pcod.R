...
catch_dat<-read.csv(paste(datadir,"gfcatchbyfishery_withnewfields_pcod.csv",sep=""),header=T)
...
catch_dat$SAMPLING_STRATA_NAME<-str_replace(catch_dat$SAMPLING_STRATA_NAME,
                                            ...
                                            "HAL EM")
catch_dat$SAMPLING_STRATA_NAME<-str_replace(catch_dat$SAMPLING_STRATA_NAME,
                                            "Tender Hook-and-line Trip Selection",
                                            "HAL Tender PC")

#subset trip specific info
c2<-catch_dat[,c("YEAR","WEEK_ENDING_DATE","VESSEL_FEDERAL_PERMIT","VESSEL_ADFG","VESSEL_NAME","VESSEL_LENGTH","TRIP_TARGET_NAME",
                 "PROCESSOR_FEDERAL_PERMIT","HARVEST_SECTOR","DEPLOYMENT_CATEGORY","SAMPLING_STRATA_NAME")]

# Catch Summary ----
# proportion of catch by area and season
catch_props<-ddply(catch_dat,c("YEAR","FMP_AREA","NMFS_AREA"),summarize,
                   catch=sum(CATCH_MT))
...
ggplot(catch_dat,aes(x=YEAR,y=CATCH_MT,fill=SAMPLING_STRATA_NAME))+
  ...
  facet_grid(FMP_SUBAREA~.,scales="free_y")+
  labs(y="Proportion",title="Total Catch by Observer Strata - Pacific cod")
# take out non-targets then look by area and FMP 
ggplot(catch_dat[catch_dat$TRIP_TARGET_GROUP=="Pacific Cod",],aes(x=YEAR,y=CATCH_MT,fill=SAMPLING_STRATA_NAME))+
  geom_bar(position="stack", stat="identity")+
  scale_y_continuous(breaks=scales::pretty_breaks((n=3)))+
  facet_grid(FMP_SUBAREA~., scales="free_y")+
  labs(y="Catch (mt)",title="Targeted Fishery by FMP Subarea - Pacific Cod")

# Get biological data ----
biodat<-read.csv(paste(datadir,"norpac_age_report_flattened_Pcod.csv",sep=""),header=T, skip=7)
...
bio_obs$FMP.Subarea <- factor(bio_obs$FMP.Subarea, 
                              levels = c("AI","BS","WG","CG","WY","SE"))
write.csv(bio_obs,paste(cleandatdir,"biosampls_w_obs_Pcod.csv",sep=""),row.names = F)
...
ggplot(ct_l[ct_l$Year>=2015,],aes(x=Year,y=n_samps,fill=SAMPLING_STRATA_NAME))+
  geom_bar(position="fill", stat="identity")+
  labs(y="Proportion",title="Proportion Lengths by Observer Strata - Pacific Cod")
ggplot(ct_l[ct_l$Year>=2015,],aes(x=Year,y=n_samps,fill=SAMPLING_STRATA_NAME))+
  geom_bar(position="stack", stat="identity")+
  labs(y="n lengths",title="Lengths by Observer Strata - Pacific Cod")
...
ggplot(ct_la[ct_la$Year>=2015,],aes(x=Year,y=n_samps,fill=SAMPLING_STRATA_NAME))+
  geom_bar(position="stack", stat="identity")+
  facet_grid(FMP.Subarea~., scales="free_y")+
  labs(y="n lengths",title="Lengths by Observer Strata - Pacific Cod")
...
ggplot(sp2_bio[sp2_bio$Year>=2015,],aes(x=Year,y=n_samps,fill=FMP.Subarea))+
  geom_bar(position="stack", stat="identity")+
  labs(y="n lengths",title="Lengths by FMP Subarea - Pacific Cod")
